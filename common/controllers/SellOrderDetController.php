...
class SellOrderDetController extends Controller {
    ...
    public function actionCreate() {
        $model = new SellOrderDet;

        // Uncomment the following line if AJAX validation is needed
        // $this->performAjaxValidation($model);

        if (isset($_POST['SellOrderDet'])) {
            $model->attributes = $_POST['SellOrderDet'];
            if ($model->save())
...
//        $session->open();
        ...
        $model = new SellOrderDet('search');
        $model->unsetAttributes();  // clear any default values

        if (isset($_GET['SellOrderDet'])) {
            $model->attributes = $_GET['SellOrderDet'];
...
//        $session['SellOrderDet_records'] = SellOrderDet::model()->findAll($criteria);
    ...
    public function loadModel($id) {
        $model = SellOrderDet::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'sell-order-det-form') {
